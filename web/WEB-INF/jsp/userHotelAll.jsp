...
<div class="control-group" style="border: 10px solid rgba(0,128,0,0.5); width: 1000px;height: 1000px;margin-bottom:0px;margin: 0 auto;">
    ...
    <table id="commentTable">
        <c:forEach items="${hotelAll}" var="h">
            <tr onclick="location.href='/Travel/user/toHotelDetails?hotelId=${h.hotelId}'">
                <td>${h.hotelId}&nbsp;
                    <span style="margin-left: 850px;">👍<span id="${h.hotelId}">${h.hotelScore}</span></span>
               <br><br><br>
                    <a href="user/toHotelDetails?hotelId=${h.hotelId}">${h.hotelName}</a></td>
            </tr>
        </c:forEach>
    </table>
    <br><br><br>
    <form id="paging" style="text-align:center;">
        <tr>
            <td colspan="8" align="center" >共${page2.totalRecords}条记录 共${page2.totalPages}页 当前第${page2.pageNo}页
                ...
                <a href="user/hotelAll?pageNo=${page2.topPageNo}"><input type="button" name="fristPage" value="首页" /></a>

                <c:choose>
                    <c:when test="${page2.pageNo!=1}">

                        <a href="user/hotelAll?pageNo=${page2.previousPageNo}"><input type="button" name="previousPage" value="上一页" /></a>
                ...
                <c:choose>
                    <c:when test="${page2.pageNo!= page2.totalPages}">
                        <a href="user/hotelAll?pageNo=${page2.nextPageNo}"><input type="button" name="nextPage" value="下一页" /></a>
                    </c:when>
                    <c:otherwise>
                ...
                <a href="user/hotelAll?pageNo=${page2.bottomPageNo }"><input type="button" name="lastPage" value="尾页" /></a>
            </td>
        </tr>
    </form>
